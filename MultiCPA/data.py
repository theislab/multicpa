...
class Dataset:
    def __init__(self,
        ...
        self.raw_proteins_key = raw_proteins_key

        if counts_key is not None:
            self.raw_genes = torch.Tensor(data.layers[counts_key].A)

        if proteins_key is not None:
            self.proteins = torch.Tensor(data.obsm[proteins_key])
            if raw_proteins_key is not None:
                self.raw_proteins = torch.Tensor(data.obsm[raw_proteins_key])
        else:
            self.proteins = None
        ...
        if self.proteins is not None:
            self.num_proteins = self.proteins.shape[1]
        else:
            self.num_proteins = 0
    ...
    def __getitem__(self, i):
        ...
        raw_genes_ = None
        if self.counts_key is not None:
            raw_genes_ = self.raw_genes[i]

        proteins_ = None
        if self.proteins_key is not None:
            proteins_ = self.proteins[i]

        raw_proteins_ = None
        if self.raw_proteins_key is not None:
            raw_proteins_ = self.raw_proteins[i]

        return self.genes[i], self.drugs[i], self.cell_types[i], proteins_, raw_genes_, raw_proteins_
...
class SubDataset:
    ...
    def __init__(self, dataset, indices):
        ...
        self.counts_key = dataset.counts_key
        self.proteins_key = dataset.proteins_key
        self.raw_proteins_key = dataset.raw_proteins_key
        ...
        if dataset.proteins_key is not None:
            self.proteins = dataset.proteins[indices]

        if dataset.raw_proteins_key is not None:
            self.raw_proteins = dataset.raw_proteins[indices]

        if dataset.counts_key is not None:
            self.raw_genes = dataset.raw_genes[indices]
